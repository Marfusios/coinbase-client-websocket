...
namespace Coinbase.Client.Websocket.Validations
{
    internal static class ConValidations
    {
        ...
        public static void ValidateInput(string value, string name)
        {
            ...
            {
                throw new CoinbaseBadInputException($"Input string parameter '{name}' is null or empty. Please correct it.");
            }
        }
        ...
        public static void ValidateInput(int value, string name, int minValue = int.MinValue, int maxValue = int.MaxValue)
        {
            if (value < minValue)
            {
                throw new CoinbaseBadInputException($"Input parameter '{name}' is lower than {minValue}. Please correct it.");
            }
            if (value > maxValue)
            {
                throw new CoinbaseBadInputException($"Input parameter '{name}' is higher than {maxValue}. Please correct it.");
            }
        }
